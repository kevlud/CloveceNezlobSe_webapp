...
export class Board {
    ...
    thrownDiceNumber: number = 6;
    currentlyDragged?: PlayerFigure;
    turnMovement: FigureMovement[] = [];
    ...
    constructor(playerColor: string) {
        ...
        this.playerColor = <FigureColor>playerColor;

        this.initBoardMap();
    }

    private initBoardMap() {
    ...
    updateMovedFigures(moves: FigureMovement[]) {
    ...
    startedDraggingFigure = (figureId: string) => {
    ...
    droppedFigureAt = (boardPositionId: string): boolean => {
        if (boardPositionId == null) return false;

        let targetBoardPosition: BoardPosition = this.boardMap.findBoardPositionByIdInMyPath(boardPositionId);
        //console.log(this.boardMap.myPathToWin)
        if (targetBoardPosition == null) return false;
    ...
    private isValidMove(currentlyDragged: PlayerFigure, targetBoardPosition: BoardPosition): boolean {
        ...
        if (currentBoardPosition.type == PositionType.PlayerHolder && this.thrownDiceNumber == 6) {
            ...
            let nextBoardPosition: BoardPosition = currentBoardPosition;
    ...
    private moveOtherFigureToHolder(sittingFigure: Figure) {
...
export class BoardPosition {
    ...
    public next?: BoardPosition;

    readonly type: PositionType;
    public sittingFigure?: Figure;


    constructor(id: string, type: PositionType, sittingFigure?: Figure) {
        this.svgId = id;
        this.type = type;
...
export class FigureMovement {
    ...
    public movedToId: string

    constructor(figureId: string, movedToId: string) {
...
export class BoardMap {
    ...
    public yellowHolders: BoardPosition[] = [];
    public yellowFinishes: BoardPosition[] = [];

    public redHolders: BoardPosition[] = [];
    public redFinishes: BoardPosition[] = [];

    public greenHolders: BoardPosition[] = [];
    public greenFinishes: BoardPosition[] = [];

    public blueHolders: BoardPosition[] = [];
    public blueFinishes: BoardPosition[] = [];

    public boardPositions: BoardPosition[] = [];
    public figures: Figure[] = [];

    public myPathToWin: BoardPosition[] = [];
    ...
    public generateMyPath(color: FigureColor) {

        if (color == FigureColor.Yellow) {
            for (let i = 0; i < 40; i++) {
                this.myPathToWin.push(this.boardPositions[i])
                if (i != 0) this.myPathToWin[i - 1].next = this.myPathToWin[i];
            }
        } else if (color == FigureColor.Red) {
            for (let i = 10; i < 40; i++) {
                this.myPathToWin.push(this.boardPositions[i]);
                if (i != 10) this.myPathToWin[i - 1 - 10].next = this.myPathToWin[i - 10];
            }
            for (let i = 0; i < 10; i++) {
                this.myPathToWin.push(this.boardPositions[i]);
            ...
            for (let i = 20; i < 40; i++) {
                this.myPathToWin.push(this.boardPositions[i]);
                if (i != 20) this.myPathToWin[i - 1 - 20].next = this.myPathToWin[i - 20];
            }
            for (let i = 0; i < 20; i++) {
                this.myPathToWin.push(this.boardPositions[i]);
            ...
            for (let i = 30; i < 40; i++) {
                this.myPathToWin.push(this.boardPositions[i]);
                if (i != 30) this.myPathToWin[i - 1 - 30].next = this.myPathToWin[i - 30];
            }
            for (let i = 0; i < 30; i++) {
                this.myPathToWin.push(this.boardPositions[i]);
    ...
    findBoardPositionById(boardPositionId: string): BoardPosition {
    ...
    findBoardPositionByIdInMyPath(boardPositionId: string): BoardPosition {
        for (let boardPosition of this.myPathToWin)
            if (boardPosition.svgId == boardPositionId)
                return boardPosition;
    }

    findFigureById(figureId: string): Figure {
